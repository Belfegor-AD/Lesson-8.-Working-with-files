...
def copy_contact():
    ...
    while int(index) < 1 or int(index) > len(contacts_list):
        print('Некорректный номер строки!')
        index = input('Введите номер строки для копирования: ')
    
    contact_to_copy = contacts_list[int(index)-1]
    
    with open("copied_phonebook.txt", 'a', encoding='utf-8') as copied_file:
        copied_file.write(contact_to_copy + '\n\n')
    
    print('Контакт успешно скопирован в новый файл.')
